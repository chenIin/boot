...
$(document).ready(function() {
	$('#testCountryTable').bootstrapTable({
		  ...
		  //请求方法
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
                     ...
                     <% }else if(shiro.hasPermission("test:grid:testCountry:view")){ %>
					   if(!value){
			...
			,{
			   ...
			   formatter:  function operateFormatter(value, row, index) {
				   return [
					<% if(shiro.hasPermission("test:grid:testCountry:view")){ %>
	 ...
	 $("#btnImportExcel").click(function(){//导入Excel
		 ...
		 jp.uploadFile(importForm,"${ctx}/test/grid/testCountry/import",function (data) {
			 if(data.success){
	...
	$("#export").click(function(){//导出Excel文件
            ...
            var sortName = $('#testCountryTable').bootstrapTable("getOptions", "none").sortName;
            var sortOrder = $('#testCountryTable').bootstrapTable("getOptions", "none").sortOrder;
            var values = "";
			...
			jp.downloadFile('${ctx}/test/grid/testCountry/export?'+values);
	  })
  ...
  function del(ids){
	 ...
	 jp.confirm('确认要删除该国家记录吗？', function(){
		...
		jp.get("${ctx}/test/grid/testCountry/delete?ids=" + ids, function(data){
				if(data.success){
					refresh();
					jp.toastr_success(data.msg);
				}else{
  ...
  function refresh() {
   ...
   //新增表单页面
 function add() {
     jp.openSaveDialog('新增国家', "${ctx}/test/grid/testCountry/form/add",'800px', '500px');
 }
  //编辑表单页面
  function edit(id){
      if(!id){
	  ...
	  jp.openSaveDialog('编辑国家', "${ctx}/test/grid/testCountry/form/edit?id="+id,'800px', '500px');
  }
  //查看表单页面
  function view(id) {
      if(!id){
      ...
      jp.openViewDialog('查看国家', "${ctx}/test/grid/testCountry/form/view?id="+id,'800px', '500px');
  }
</script>