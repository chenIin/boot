...
$(document).ready(function() {
	$('#courseTable').bootstrapTable({
		  ...
		  //请求方法
			...
			,{
				...
				if(!value){
					...
					return "<a  href='#' onclick='edit(\""+row.id+"\")'>"+value+"</a>";
                     	...
                     	<% }else if(shiro.hasPermission("test:manytomany:course:view")){ %>
		     		if(!value){
			...
			,{
			   ...
			   formatter:  function operateFormatter(value, row, index) {
				   return [
					<% if(shiro.hasPermission("test:manytomany:course:view")){ %>
	 ...
	 $("#btnImportExcel").click(function(){//导入Excel
		 ...
		 jp.uploadFile(importForm,"${ctx}/test/manytomany/course/import",function (data) {
			 if(data.success){
	 ...
	 $("#export").click(function(){//导出Excel文件
            ...
            var sortName = $('#courseTable').bootstrapTable("getOptions", "none").sortName;
            var sortOrder = $('#courseTable').bootstrapTable("getOptions", "none").sortOrder;
            var values = "";
			...
			jp.downloadFile('${ctx}/test/manytomany/course/export?'+values);
	  })
  ...
  function del(ids){
	 ...
	 jp.confirm('确认要删除该课程记录吗？', function(){
		...
		jp.get("${ctx}/test/manytomany/course/delete?ids=" + ids, function(data){
				if(data.success){
					refresh();
					jp.toastr_success(data.msg);
				}else{
