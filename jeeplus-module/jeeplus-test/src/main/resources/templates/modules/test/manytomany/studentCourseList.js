...
$(document).ready(function() {
	$('#studentCourseTable').bootstrapTable({
		  ...
		  //请求方法
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
                     ...
                     <% }else if(shiro.hasPermission("test:manytomany:studentCourse:view")){ %>
					   if(!value){
			...
			,{
			   ...
			   formatter:  function operateFormatter(value, row, index) {
				   return [
					<% if(shiro.hasPermission("test:manytomany:studentCourse:view")){ %>
	 ...
	 $("#btnImportExcel").click(function(){//导入Excel
		 ...
		 jp.uploadFile(importForm,"${ctx}/test/manytomany/studentCourse/import",function (data) {
			 if(data.success){
	 ...
	 $("#export").click(function(){//导出Excel文件
            ...
            var sortName = $('#studentCourseTable').bootstrapTable("getOptions", "none").sortName;
            var sortOrder = $('#studentCourseTable').bootstrapTable("getOptions", "none").sortOrder;
            var values = "";
			...
			jp.downloadFile('${ctx}/test/manytomany/studentCourse/export?'+values);
	  })
  ...
  function del(ids){
	 ...
	 jp.confirm('确认要删除该学生课程记录记录吗？', function(){
		...
		jp.get("${ctx}/test/manytomany/studentCourse/delete?ids=" + ids, function(data){
				if(data.success){
					refresh();
					jp.toastr_success(data.msg);
				}else{
