...
$(document).ready(function() {
	$('#studentTable').bootstrapTable({
		  ...
		  //请求方法
			...
			,{
		        ...
		        <% if(shiro.hasPermission("test:manytomany:student:edit") ){ %>
				if(!value){
					return "<a  href='#' onclick='edit(\""+row.id+"\")'>-</a>";
				}else{
					return "<a  href='#' onclick='edit(\""+row.id+"\")'>"+value+"</a>";
				}
                     	<% }else if(shiro.hasPermission("test:manytomany:student:view")){ %>
		     		if(!value){
			...
			,{
			   ...
			   formatter:  function operateFormatter(value, row, index) {
				   return [
					<% if(shiro.hasPermission("test:manytomany:student:view")){ %>
				   ...
				   <% } %>
				   <% if(shiro.hasPermission("test:manytomany:student:del")){ %>
	 ...
	 $("#btnImportExcel").click(function(){//导入Excel
		 ...
		 jp.uploadFile(importForm,"${ctx}/test/manytomany/student/import",function (data) {
			 if(data.success){
	 ...
	 $("#export").click(function(){//导出Excel文件
			...
			jp.downloadFile('${ctx}/test/manytomany/student/export?'+values);
	  })
  ...
  function del(ids){
	 ...
	 jp.confirm('确认要删除该学生记录吗？', function(){
		...
		jp.get("${ctx}/test/manytomany/student/delete?ids=" + ids, function(data){
				if(data.success){
					refresh();
					jp.toastr_success(data.msg);
				}else{
