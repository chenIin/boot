...
$(document).ready(function() {
	$('#testPicTable').bootstrapTable({
		  ...
		  //请求方法
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
		        	  <% if(shiro.hasPermission("test:pic:testPic:edit") ){ %>
					   if(!value){
						  return "<a  href='#' onclick='edit(\""+row.id+"\")'>-</a>";
					   }else{
						  return "<a  href='#' onclick='edit(\""+row.id+"\")'>"+value+"</a>";
						}
                     <% }else if(shiro.hasPermission("test:pic:testPic:view")){ %>
					   if(!value){
			...
			,{
			   ...
			   formatter:  function operateFormatter(value, row, index) {
				   return [
					<% if(shiro.hasPermission("test:pic:testPic:view")){ %>
				   ...
				   <% } %>
				   <% if(shiro.hasPermission("test:pic:testPic:del")){ %>
	 ...
	 $("#btnImportExcel").click(function(){//导入Excel
		 ...
		 jp.uploadFile(importForm,"${ctx}/test/pic/testPic/import",function (data) {
			 if(data.success){
	...
	$("#export").click(function(){//导出Excel文件
            ...
            var sortName = $('#testPicTable').bootstrapTable("getOptions", "none").sortName;
            var sortOrder = $('#testPicTable').bootstrapTable("getOptions", "none").sortOrder;
            var values = "";
			...
			jp.downloadFile('${ctx}/test/pic/testPic/export?'+values);
	  })
  ...
  function del(ids){
	 ...
	 jp.confirm('确认要删除该图片管理记录吗？', function(){
		...
		jp.get("${ctx}/test/pic/testPic/delete?ids=" + ids, function(data){
				if(data.success){
					refresh();
					jp.toastr_success(data.msg);
				}else{
  ...
  function refresh() {
   ...
   //新增表单页面
 function add() {
     jp.openSaveDialog('新增图片管理', "${ctx}/test/pic/testPic/form/add",'800px', '500px');
 }
  //编辑表单页面
  function edit(id){
      if(!id){
	  ...
	  jp.openSaveDialog('编辑图片管理', "${ctx}/test/pic/testPic/form/edit?id="+id,'800px', '500px');
  }
  //查看表单页面
  function view(id) {
      if(!id){
      ...
      jp.openViewDialog('查看图片管理', "${ctx}/test/pic/testPic/form/view?id="+id,'800px', '500px');
  }
</script>