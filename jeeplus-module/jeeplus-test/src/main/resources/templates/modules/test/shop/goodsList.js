...
$(document).ready(function() {
	$('#goodsTable').bootstrapTable({
		  ...
		  //请求方法
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
		        	  <% if(shiro.hasPermission("test:shop:goods:edit") ){ %>
					   if(!value){
						  return "<a  href='#' onclick='edit(\""+row.id+"\")'>-</a>";
					   }else{
						  return "<a  href='#' onclick='edit(\""+row.id+"\")'>"+value+"</a>";
						}
                     <% }else if(shiro.hasPermission("test:shop:goods:view")){ %>
					   if(!value){
			...
			,{
			   ...
			   formatter:  function operateFormatter(value, row, index) {
				   return [
					<% if(shiro.hasPermission("test:shop:goods:view")){ %>
				   ...
				   <% } %>
				   <% if(shiro.hasPermission("test:shop:goods:del")){ %>
	 ...
	 $("#btnImportExcel").click(function(){//导入Excel
		 ...
		 jp.uploadFile(importForm,"${ctx}/test/shop/goods/import",function (data) {
			 if(data.success){
	 ...
	 $("#btnDownloadTpl").click(function(){//下载模板文件
            jp.downloadFile('${ctx}/test/shop/goods/import/template');
		})
	...
	$("#export").click(function(){//导出Excel文件
			...
			jp.downloadFile('${ctx}/test/shop/goods/export?'+values);
	  })
  ...
  function del(ids){
	 ...
	 jp.confirm('确认要删除该商品记录吗？', function(){
		...
		jp.get("${ctx}/test/shop/goods/delete?ids=" + ids, function(data){
				if(data.success){
					refresh();
					jp.toastr_success(data.msg);
				}else{
  ...
  function refresh() {
   ...
   //新增表单页面
 function add() {
     jp.openSaveDialog('新增商品', "${ctx}/test/shop/goods/form/add",'800px', '500px');
 }
  //编辑表单页面
  function edit(id){
      if(!id){
	  ...
	  jp.openSaveDialog('编辑商品', "${ctx}/test/shop/goods/form/edit?id="+id,'800px', '500px');
  }
  //查看表单页面
  function view(id) {
      if(!id){
      ...
      jp.openViewDialog('查看商品', "${ctx}/test/shop/goods/form/view?id="+id,'800px', '500px');
  }
</script>