	...
	var CONSTANTS = {
	  ...
	  4: {
	    ...
	    html: {
	      ...
	      searchInput: '<input class="%s%s" type="text" placeholder="%s">',
	      searchButton: '<button class="btn btn-white" type="button" name="search" title="%s">%s %s</button>',
	      searchClearButton: '<button class="btn btn-white" type="button" name="clearSearch" title="%s">%s %s</button>'
	    }
	  }
	}[bootstrapVersion];
	var DEFAULTS = {
	  ...
	  _createClass(BootstrapTable, [{
	    ...
	    value: function initToolbar() {
	      ...
	      if (typeof o.icons === 'string') {
			...
			if (o.showSearch) {
				html.push("<button class=\"".concat(this.constants.buttonsClass, "\" type=\"button\" name=\"showSearch\"\n        aria-label=\"Search\" title=\"").concat(o.formatShowSearch(), "\">\n        ").concat(o.showButtonIcons ? Utils.sprintf(this.constants.html.icon, o.iconsPrefix, o.icons.showSearch) : '', "\n        ").concat(o.showButtonText ? o.formatShowSearch() : '', "\n        </button>"));
			}
	    ...
	    value: function initRow(item, i, data, trFragments) {
	      ...
	      this.header.fields.forEach(function (field, j) {
	        ...
	        if (column.checkbox || column.radio) {
	          ...
	          text = [_this7.options.cardView ? "<div class=\"card-view ".concat(c, "\">") : "<td class=\"bs-checkbox ".concat(c, "\"").concat(class_).concat(style_, ">"), "<label class='bs-checkbox'>\n            <input\n            data-index=\"".concat(i, "\"\n            name=\"").concat(_this7.options.selectItemName, "\"\n            type=\"").concat(type, "\"\n            ").concat(Utils.sprintf('value="%s"', item[_this7.options.idField]), "\n            ").concat(Utils.sprintf('checked="%s"', isChecked ? 'checked' : undefined), "\n            ").concat(Utils.sprintf('disabled="%s"', isDisabled ? 'disabled' : undefined), " />\n            <span></span>\n            </label>"), _this7.header.formatters[j] && typeof value === 'string' ? value : '', _this7.options.cardView ? '</div>' : '</td>'].join('');
