...
(function($) {
    ...
    BootstrapTreeTable.prototype.initToolbar = function() {
        ...
        if (self.options.showRefresh) {
            var $refreshBtn = $('<button class="btn btn-white btn-outline refresh_btn" type="button" aria-label="refresh" title="刷新"><i class="'+self.options.toolRefreshClass+'"></i></button>');
            $rightToolbar.append($refreshBtn);
        }
        // 是否显示列选项
        if (self.options.showColumns) {
            var $columns_div = $('<div class="btn-group pull-right" title="列"><button type="button" aria-label="columns" class="btn btn-white btn-outline dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="'+self.options.toolColumnsClass+'"></i> <span class="caret"></span></button></div>');
            ...
            $.each(self.leftFixedColumns, function(i, column) {
                if (!(column.checkbox || column.radio)) {
                    ...
                    var _li = $('<li role="menuitem"><label class="bootstrap-treetable-checkbox"><input type="checkbox" checked="checked" disabled data-field="' + column.field + '" value="' + column.field + '" > ' + column.title + ' <span></span></label></li>');
    ...
    BootstrapTreeTable.prototype.renderRow = function(item, isP, lv, row_id, p_id) {
        ...
        $.each(_columns, function(index, column) {
            // 判断是不是选择列
            if (column.checkbox || column.radio) {
                var $td = $('<td style="text-align:center;width:57px"></td>');
                if (column.radio) {
                    var $ipt = $('<label class="bootstrap-treetable-radio"> <input name="select_item" type="radio" value="' + item[self.options.id] + '"></input><span></span></label>');
                    $td.append($ipt);
                }
                if (column.checkbox) {
                    var $ipt = $('<label class="bootstrap-treetable-checkbox"> <input name="select_item" type="checkbox" value="' + item[self.options.id] + '"></input><span></span></label>');
                    $td.append($ipt);
                }
                $tr.append($td);
            } else {
    ...
    BootstrapTreeTable.prototype.selectedDataIdsManager = function(itemVal,isCheckbox,isAdd) {
        ...
        if(!(isCheckbox)){
            ...
            if(self.hasFixedColumn){
                ...
                $leftIpt.parent().parent().parent().addClass("treetable-selected");
                $leftIpt.prop('checked', true);
            }
            self.$el.find("tbody").find("tr").removeClass("treetable-selected");
            $ipt.parent().parent().parent().addClass("treetable-selected");
            ...
            if(isAdd){
                ...
                if (_index > -1) {
                    ...
                    if(self.hasFixedColumn){
                        $leftIpt.parent().parent().parent().removeClass("treetable-selected");
                        $leftIpt.prop('checked', "");
                    }
                    $ipt.parent().parent().parent().removeClass("treetable-selected");
                    $ipt.prop('checked', "");
                }
            }
        }
    ...
    BootstrapTreeTable.DEFAULTS = {
        ...
        toolColumnsClass: 'fa fa-th-list', // 工具栏列按钮
        onAll: function(data) {
            return false;
        },
