			...
			,{
			    ...
			    formatter:function (value, row, index) {
				   if(value == "[已挂起]" || value =="[已结束]" || value =="[用户撤销]" || value.startsWith("[流程作废]")){
				   		return "<font class='badge badge-danger'>" + value +"</font>";
				   }else if(value == "[正常结束]"){
					   return "<font class='badge badge-success' '>" + value +"</font>";
				   }else{
                       return "<font class='badge badge-default' '>" + value +"</font>";
				   }
			    }
                       ...
                       events: {
                           ...
                           formatter: function operateFormatter(value, row, index) {
                               if (row['procIns.status'].startsWith("[进行中]")) {
                                   return [
                                       '<a class="history btn btn-xs btn-custom waves-effect waves-light m-r-5" href="#" >进度</a>',
                                       '<a class="btn btn-xs btn-success waves-effect waves-light m-r-5" href="${ctx}/act/task/formDetail?'+'&procInsId='+row['procIns.processInstanceId']+'&procDefId='+row['procDef.processDefinitionId']+'">详情</a>',
                                        '<a href="#" class="del btn btn-xs btn-danger waves-effect waves-light">撤销</a>'
                                   ].join('');
                               } else {
                                   return [
                                       '<a class="history btn btn-xs btn-custom waves-effect waves-light m-r-5" href="#" >进度</a>',
                                       '<a class="btn btn-xs btn-success waves-effect waves-light" href="${ctx}/act/task/formDetail?'+'&procInsId='+row['procIns.processInstanceId']+'&procDefId='+row['procDef.processDefinitionId']+'">详情</a>',
                                   ].join('');
                               }
